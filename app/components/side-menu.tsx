...
export default function SideMenu() {
  ...
  const menuItems = [
    ...
    { name: "Bank", icon: <LandmarkIcon size={20} /> },
  ];
  ...
  return (
    <nav className="hidden w-full flex-col items-end gap-5 p-5 xl:flex">
      {menuItems.map((item, index) => (
        <button
          key={index}
          className="flex h-10 w-10 items-center justify-center rounded-full bg-white bg-opacity-10 active:bg-gray-400 active:text-gray-600"
        >
          {item.icon}
        </button>
      ))}
